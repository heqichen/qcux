...
│   ├── types/                   # 类型定义
│   │   ├── project.ts           # 项目数据模型
│   │   └── electron.d.ts        # Electron Bridge 类型
│   ├── store/                   # Zustand 状态管理
...
│   │       └── PageDesignView.tsx
│   ├── canvas/                  # Canvas 渲染引擎
│   │   ├── InteractionCanvas.tsx # 交互画布渲染
│   │   ├── PageDesignCanvas.tsx  # 页面设计画布渲染
│   │   ├── hitTest.ts           # 碰撞检测工具
│   │   ├── interactionCanvasUtils.ts # 交互画布组合渲染
│   │   ├── pageDesignCanvasUtils.ts  # 页面设计画布组合渲染
│   │   └── renderers/           # 各类型元素渲染器
│   │       ├── pageRenderer.ts
│   │       ├── objectRenderer.ts
│   │       ├── textRenderer.ts
...
│   ├── hooks/                   # 自定义 Hooks
│   │   └── useProjectSave.ts    # 共享保存逻辑
│   ├── services/                # 业务逻辑层
│   │   ├── exportHtml.ts        # 项目导出逻辑
│   │   └── projectFileService.ts # 文件读写 (网页 / Electron)
│   ├── components/              # 可复用 UI 组件
│   │   ├── ProjectStatus.tsx    # 项目路径/未保存状态
│   │   └── ToolbarLayout.tsx    # 共享工具栏布局
│   └── utils/                   # 工具函数
│       ├── id.ts                # ID 生成
│       ├── geometry.ts          # 几何计算
│       └── constants.ts         # 常量定义
...
<App>
  ...
  │       ├── [未选中] → 界面名称输入框
  │       ├── <ObjectProperties /> + [删除元素]
  │       └── <Text/Button Properties /> + [删除元素]
  │
  ├── <AddPageDialog />              (模态框)
...
Ctrl/Cmd + C → 复制当前选中页面
Ctrl/Cmd + V → 粘贴页面副本（新页面与内部元素重新生成 ID，不复制 links）
```
...
Delete → 删除当前选中元素
Ctrl/Cmd + C → 复制当前选中元素
Ctrl/Cmd + V → 粘贴元素副本（向右下偏移并自动选中新元素）
```
...
复制元素时会分配新的 zIndex，并按照粘贴次数累加固定偏移量
```
